...
ML {* Diabelli.strict_snf_to_sd @{term "(\<exists>s1 s2. distinct[s1, s2] \<and> s1 \<in> A \<inter> B \<and> s2 \<in> (A - B) \<union> (B - A)) \<longrightarrow> (\<exists>s1 s2. distinct[s1, s2] \<and> s1 \<in> (A - B) \<union> (A \<inter> B) \<and> s2 \<in> (B - A) \<union> (A \<inter> B))"} *}
