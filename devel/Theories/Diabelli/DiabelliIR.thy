...
apply sd_tac
proof -
  ML_prf {* Diabelli.speedith_batch_apply @{term "(\<exists>s1 s2. distinct[s1, s2] \<and> s1 \<in> A \<inter> B \<and> s2 \<in> (A - B) \<union> (B - A)) \<longrightarrow> (\<exists>s1 s2. distinct[s1, s2] \<and> s1 \<in> A \<and> s2 \<in> B)"} NONE [] *}
  show ?thesis
    apply(sd_tac)
    apply(auto)
    by (iprover)
